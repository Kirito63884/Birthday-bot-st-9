...
def calculate_age(birth_date):
    """
    Рассчитывает возраст на основе даты рождения (ДД.ММ.ГГГГ)
    Возвращает: возраст (int) и текст с пояснением
    """
    ...
    except ValueError:  # Если 29 февраля
        birthday = datetime(year, 3, 1)  # Используем 1 марта
...
def get_next_birthday(birth_date):
    """Возвращает дату следующего дня рождения"""
    today = datetime.now(TIMEZONE)
    day, month, year = map(int, birth_date.split('.'))
    
    # Создаём дату рождения в этом году
    try:
    ...
    if birthday_this_year < today:
        try:
            ...
            birthday_this_year = datetime(today.year + 1, 3, 1)
...
async def bd_help(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value="""
        `/force_check` — Принудительная проверка именинников
        """,
        inline=False
    )
    ...
    embed.add_field(
        ...
        value="""
        **Добавить свой ДР:** `/add_bd 15.07.2000`
        **Добавить ДР друга:** `/add_bd 15.07.2000 @Пользователь`
        **Удалить свой ДР:** `/remove_bd`
        **Удалить ДР друга:** `/remove_bd @Пользователь`
        **Узнать возраст:** `/age @Пользователь`
        """,
        inline=False
    )
    ...
    embed.add_field(
        ...
        • Автоматическое поздравление именинников с указанием возраста
    ...
    embed.add_field(
        name="📌 **Примеры:**",
        value="""
        `🔹 /add_bd 25.12.1995` — Добавить свой ДР 25 декабря 1995 года
        `🔹 /add_bd 01.01.2000 @Иван` — Добавить ДР Ивана 1 января 2000 года
        `🔹 /list_bd` — Показать всех именинников
        `🔹 /age @Петя` — Узнать возраст Пети
        """,
        inline=False
    )
...
async def add_birthday(interaction: discord.Interaction, date: str, user: discord.User = None):
    ...
    # Проверка года (не в будущем)
    current_year = datetime.now(TIMEZONE).year
    if year > current_year:
        embed = create_embed(
            "❌ Ошибка!",
            "Год рождения не может быть в будущем!",
            COLORS['error']
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    if year < 1900:
        embed = create_embed(
            "❌ Ошибка!",
            "Слишком старый год рождения! Укажите год от 1900.",
            COLORS['error']
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
...
async def my_birthday(interaction: discord.Interaction):
    ...
    try:
        ...
        # Проверяем, был ли уже ДР в этом году
        day, month, year = map(int, date.split('.'))
        today = datetime.now(TIMEZONE)
        if (today.month, today.day) >= (month, day):
            status = f"✅ Уже исполнилось **{age_text}**"
        else:
            status = f"⏳ Скоро исполнится **{age_text}**"
            
    except:
        age_text = "неизвестно"
        status = "❓ Не удалось рассчитать возраст"
...
async def get_age(interaction: discord.Interaction, user: discord.User = None):
    """Показывает возраст пользователя"""
    if user is None:
        user = interaction.user
    ...
    if user_id not in data:
        embed = create_embed(
            "❌ Не найден",
            f"День рождения для **{user.display_name}** не добавлен в базу!",
            COLORS['error']
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        day, month, year = map(int, date.split('.'))
        today = datetime.now(TIMEZONE)
        has_passed = (today.month, today.day) >= (month, day)
        
        if has_passed:
            status = f"✅ Уже исполнилось **{age_text}**"
        else:
            status = f"⏳ Скоро исполнится **{age_text}** (в этом году)"
...
async def check_birthdays():
    ...
    for user_id, info in data.items():
        # Проверяем только день и месяц (год не учитываем)
        date_parts = info['date'].split('.')
...
async def ping(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        color=COLORS['success'] if latency < 100 else COLORS['warning'] if latency < 300 else COLORS['error'],
        timestamp=datetime.now(TIMEZONE)
    )
    ...
    embed.add_field(name="⏱️ Время", value=f"{datetime.now(TIMEZONE).strftime('%H:%M:%S')}", inline=True)
    embed.set_footer(text="Бот работает стабильно ✅")
